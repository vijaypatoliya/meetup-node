var debug = require('debug')('meetup:Resource');
...
// Provide extension mechanism for meetup Resource Sub-Classes
meetupResource.extend = utils.protoExtend;

// Expose method-creator & prepared (basic) methods
meetupResource.methods = require('./MeetUpHttpClient');

/**
 * Encapsulates request logic for a meetup Resource
 */
function meetupResource(meetup, urlData) {
  this._meetup = meetup;
  this._urlData = urlData || {};

  this.basePath = utils.makeURLInterpolator(meetup.getApiField('basePath'));
  this.path = utils.makeURLInterpolator(this.path);

  // if (this.includeBasic) {
  //   this.includeBasic.forEach(function (methodName) {
  //     this[methodName] = meetupResource.methods[methodName];
  //   }, this);
  // }
...
meetupResource.prototype = {
  ...
  // over how a meetupResource's request data will get converted into an HTTP
  ...
  _timeoutHandler: function (timeout, req, callback) {
    ...
    return function () {
      ...
      callback.call(self, new Error.meetupConnectionError({
      ...
      res.on('end', function () {
        ...
        if (userOptions.userCharset && userOptions.userCharset.length > 0) {
          ...
          try {
            ...
            return callback.call(self, new Error.meetupAPIError({
              message: 'Failed to parse response received from the meetup API',
        ...
        try {
          ...
          return callback.call(self, new Error.meetupAPIError({
            message: 'Invalid response received from the meetup API',
            StatusCode: statusCode || 'unknown'
          }), null);
        }
      });
  ...
  _errorHandler: function (req, callback) {
    ...
    return function (error) {
      ...
      callback.call(self, new Error.meetupConnectionError({
        message: 'An error occurred with our connection to meetup',
        error: error
      }), null);
    };
  },
  ...
  _request: function (method, path, data, auth, options, callback) {
    data.key = this._meetup.getApiField('key');  // set secret key as common params
    ...
    function makeRequest() {
      var timeout = self._meetup.getApiField('timeout');
      var isInsecureConnection = self._meetup.getApiField('protocol') === 'http';

      var host = self.overrideHost || self._meetup.getApiField('host');
      var params = {
        host: host,
        port: self._meetup.getApiField('port'),
        path: path,
        method: method,
        headers: headers
      };
